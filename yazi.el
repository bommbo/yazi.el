...
(defun yazi--process-output ()
  "Process yazi output and call callback."
  ...
  ;; Read output file
  (let ((output (when (and yazi--output-file 
                           (file-exists-p yazi--output-file))
                  (with-temp-buffer
    ...
    ;; Call callback if we have output
    (when (and yazi--callback output (not (string-empty-p output)))
